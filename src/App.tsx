...
import { Scene } from "./stores/models";
...
const App: React.FC = () => {
  ...
  const handleUpdateShape = (shape: any) => {
    const updateShapes = currentScene.shapes.map((item) =>
      item.id === shape.id ? { ...item, ...shape } : item
    );
    setCurrentScene({ ...currentScene, shapes: updateShapes });
  };
  return (
    <SnackbarProvider maxSnack={4} autoHideDuration={5000}>
      {/* <CanvasRenderer scene={currentScene} onUpdateShape={handleUpdateShape} /> */}
